...
- 等级、掌握率和成就由源状态派生，不独立持久化。等级门槛固定为每 100 XP 一级（`XP_PER_LEVEL`），约 5 道「掌握」升一级。
- 升级时播放一次非阻断特效：不抢焦点、不拦截输入、约 1.3 秒自动消失，`prefers-reduced-motion` 下去掉彩带只留静态徽章。同一次自评若已弹出打卡对话框，则不再叠加特效，等级信息由对话框呈现。
