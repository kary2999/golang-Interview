...
- 参考答案默认隐藏；揭晓后才出现三级自评。
- **不会**：获得 2 XP，加入待复习，7 次后续自评后再次出现。
- **模糊**：获得 8 XP，保留待复习，20 次后续自评后再次出现。
- **掌握**：获得 20 XP，并从待复习中移除。
- 跳过当前题不获得 XP，也不修改掌握统计。
- 待复习项使用独立调度队列，不会向基础题序写入重复 ID。
- 一轮结束会非阻断地结算，并自动进入下一轮；未到期的待复习项继续保留。
- 每累计 1000 XP 提升一级；XP 表示练习投入，不等同于掌握率。
- 同一浏览器本地自然日完成至少 10 次自评，该日才计入连续学习。
- “随机刷题”和“待复习”分别保存导航位置；手动重新洗牌保留 XP、统计和待复习。

主状态保存为 `go-interview-progress-v2`。首次升级时，仅在 v2 不存在且旧状态有效的情况下迁移 `go-interview-progress-v1`；旧不会题不会被静默标记为掌握。存储不可访问、损坏或写入失败时，页面会以当前内存状态继续运行并显示非阻断提示。
