...
>
> 保持数据契约：恰好 100 条，ID 按 1..100 排列且唯一；`question` 与 `answerHtml` 非空；`source` 只能是 `zhihu-archive` 或 `supplemented`。答案 HTML 只能使用提取器允许的安全标签和合法 code 语言类，禁止脚本、事件属性、样式、远程资源或可执行内容。
>
