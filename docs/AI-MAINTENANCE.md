...
- 自评按钮里的静态 `<small>` 文案由 `index.html` 独立维护，JavaScript 不会改写；调整结算或复习规则时必须同步这三处，否则界面会继续承诺已经废弃的行为。
...
- `index.html` 的 `#build-tag` 必须与 `CACHE_NAME` 的版本后缀一致，由 `verify_project.py` 强制校验；用户据此自查加载的是哪个构建，缺了这个标记就只能靠猜。
- 注册成功后以及页面重新可见时都要调用 `registration.update()`；否则旧 Worker 可能整个会话都不去检查新版本，更新提示永远不出现。
